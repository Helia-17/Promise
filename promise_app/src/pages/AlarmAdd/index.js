...
const AlarmAdd = ({navigation}) => {
    ...
    const addalarm = async ()=>{
        ...
        await enrollAlarm(title, alarmYN, selectTime1, selectTime2, selectTime3, myStartDate(), endDate, myMediList(), myTagList(), myShareList());
    }
            ...
            <ScrollView style={{ width:'100%', margin:10}} contentContainerStyle={{alignItems: 'center', margin:10}}>
                <View style={{flexDirection: "row", alignItems: 'center', width:'90%', justifyContent: 'center' , height:50}}>
                    ...
                    <View style={{width:'78%', backgroundColor:'#E9E9E9', height:40, borderRadius: 20, alignItems: 'center', justifyContent:'center'}}>
                        <TextInput onChangeText={onChangeTitle} value={title} style={{width:'80%',color:'black', backgroundColor: '#E9E9E9', borderRadius: 20, textAlign:'center'}}/>
                    </View>
                </View>
                <DateSelect selectedStart = {(data)=>setStartDate(data)} selectedEnd={(data)=>setEndDate(data)}/>
                <AddPill add = {(data)=>setAddModal(data)} ocradd = {(data)=>setOcrModal(data)} ocrdata={(data)=>setOcrPillData(data)}/>
                <Modal animationType={'fade'} transparent={true} visible={addModal}>
                    <PillModal visible={(data)=>setAddModal(data)} selected={(data)=>addList(data)}/>
                </Modal>
                <Modal animationType={'fade'} transparent={true} visible={ocrModal}>
                    <OCRModal data = {ocrPillData} selected = {(data)=>addOCRList(data)} visible={(data)=>setOcrModal(data)}/>
                </Modal>
                {myPillList()}
                <View style={{flexDirection: "row", alignItems: 'center', width:'90%', justifyContent: 'center' , height:50}}>
                    <Text style={{ fontSize:15, color:'black', fontWeight:'bold', width:'20%' }}>태그</Text>
                    <View style={{width:'78%', backgroundColor:'#E9E9E9', height:40, borderRadius: 20, alignItems: 'center', justifyContent:'center'}}>
                        <TextInput placeholder='나만의 태그를 #태그로 입력해주세요.' onChangeText={setTag} value={tag} style={{width:'80%',color:'black', backgroundColor: '#E9E9E9', borderRadius: 20, textAlign:'center'}}/>
                    </View>
                </View>
                ...
                <View style={{width:'90%', margin:10}}>
                    <TouchableOpacity style={{backgroundColor:'#A3BED7', color:'black', alignItems: 'center', borderRadius: 12, height:50, justifyContent: 'center'}} onPress={()=>addalarm()}>
